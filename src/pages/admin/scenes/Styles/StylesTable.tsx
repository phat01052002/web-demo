...
const AlertCancelDialog: React.FC<AlertDeleteDialogProps> = (props) => {
    const { t } = useTranslation();
    const { onClose, open, stylesId, onUpdate } = props;
    const store = useStore();
    const handleClose = () => {
        onClose();
    };
    const handleDelete = async () => {
        ...
        store.dispatch(change_is_loading(true));
        try {
            const res = await PostApi(`/admin/delete/styles/${stylesId}`, localStorage.getItem('token'), {});
...
const StylesTable: FC<StylesTable> = ({ initialOrigins }) => {
    const { t } = useTranslation();
    const store = useStore();
    ...
    const getDataRequestWithDraw = async () => {
        if (name.length != 24) {
            store.dispatch(change_is_loading(true));
            const res = await PostApi('/admin/get/styles-by-name', localStorage.getItem('token'), { name: name });

            if (res.data.message == 'Success') {
                setCategories(res.data.styles);
            }
            store.dispatch(change_is_loading(false));
        } else {
            store.dispatch(change_is_loading(true));
...
StylesTable.propTypes = {
    initialOrigins: PropTypes.array.isRequired,
};

StylesTable.defaultProps = {
    initialOrigins: [],
};
