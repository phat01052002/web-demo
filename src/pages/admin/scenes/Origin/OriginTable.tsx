...
const AlertCancelDialog: React.FC<AlertDeleteDialogProps> = (props) => {
    const { t } = useTranslation();
    const { onClose, open, originId, onUpdate } = props;
    const store = useStore();
    const handleClose = () => {
        onClose();
    };
    const handleDelete = async () => {
        onClose();
        store.dispatch(change_is_loading(true));
        const res = await PostApi(`/admin/delete/origin/${originId}`, localStorage.getItem('token'), {});
...
const OriginTable: FC<OriginTable> = ({ initialOrigins }) => {
    const { t } = useTranslation();
    const store = useStore();
    ...
    const getDataRequestWithDraw = async () => {
        if (name != '') {
            store.dispatch(change_is_loading(true));
            const res = await PostApi('/admin/get/origin-by-name', localStorage.getItem('token'), { name: name });

            if (res.data.message == 'Success') {
                setCategories(res.data.origins);
            }
            store.dispatch(change_is_loading(false));
        } else {
            store.dispatch(change_is_loading(true));
